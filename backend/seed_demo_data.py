...
async def run_pipeline():
    ...
    async with async_session_maker() as db:
        ...
        if not assets:
            print("No active assets found. Please run the ingestion service or check database.")
            return
        ...
        for asset in assets:
            ...
            try:
                ...
                print(f"[{asset.ticker}] Forecasting model trained and forecasts registered!")
            except Exception as e:
                print(f"[{asset.ticker}] Model training failed: {e}")

            # Step C: Run Monte Carlo simulations and build the 3D surface
            print(f"[{asset.ticker}] Step 3/3: Running Monte Carlo surface projections...")
            try:
                ...
                print(f"[{asset.ticker}] Monte Carlo projection surface built and saved!")
            except Exception as e:
        ...
        try:
            ...
            print(f"Signals evaluation complete! Created {len(signals)} total signals ({len(active_sigs)} active trades).")
        except Exception as e:
        ...
        try:
            ...
            print("Risk analytics evaluation successfully calculated and committed to DB!")
        except Exception as e:
