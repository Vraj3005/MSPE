...
async def get_results():
    async with async_session_maker() as db:
        # Get active signals
        sig_query = select(TradingSignal, Asset.ticker).join(Asset).order_by(desc(TradingSignal.timestamp)).limit(10)
        sig_res = await db.execute(sig_query)
        signals = sig_res.all()
        
        print("=== SIGNALS ===")
        for s, ticker in signals:
            print(f"Asset: {ticker} | Type: {s.signal_type} | Entry: {s.entry_price} | SL: {s.stop_loss} | TP: {s.take_profit} | Size: ${s.position_size_usd} | Active: {s.is_active}")
            
        # Get latest portfolio risk
        risk_query = select(PortfolioRiskMetrics).order_by(desc(PortfolioRiskMetrics.timestamp)).limit(1)
