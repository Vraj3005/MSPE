...
from backend.app.services.signal import PORTFOLIO_EQUITY, MAX_RISK_PER_TRADE, MAX_PORTFOLIO_RISK
...
def test_position_sizing_budget():
    ...
    assert abs(expected_size - 10000.0) < 1e-2, f"Position sizing error: {expected_size}"
    
    # Entry at 100, Stop Loss at 98 (2% drawdown - tighter stop means larger size)
    sl_tight = 98.0
    percent_loss_tight = abs(entry - sl_tight) / entry
    expected_size_tight = risk_budget_usd / percent_loss_tight # $1,000 / 0.02 = $50,000
    
    assert abs(expected_size_tight - 50000.0) < 1e-2, f"Tight position sizing error: {expected_size_tight}"
...
def test_portfolio_risk_ceiling():
    ...
    assert len(rejected) == 1, f"Expected 6th trade to be blocked, but got accepted"
    assert rejected[0]["ticker"] == "AMZN", f"Expected lowest ranked AMZN to be blocked, got {rejected[0]['ticker']}"
    assert current_active_risk == 5000.0, f"Portfolio active risk weights are incorrect: {current_active_risk}"
