...
print(
    f"  Prices: {len(prices)} points, range ${prices.min():,.0f} — ${prices.max():,.0f}"
)
...
for r in results:
    print(
        f"  {r['model_name']:30s} ret={r['expected_return']:+.4f}  vol={r['expected_volatility']:.4f}"
    )
...
print(
    f"  Validation steps: {wf.model_results[0].num_validation_steps if wf.model_results else 0}"
)
print("\n  Full ranking:")
for r in wf.model_results:
    print(
        f"    {r.model_name:30s} cal={r.calibration_score:.4f}  cov={r.interval_coverage:.4f}  dir={r.directional_accuracy:.4f}  mae={r.mae:.6f}"
    )
...
for s in proj.horizons:
    print(
        f"  {s.horizon_days:2d}d: Bear=${s.bear_price:,.2f}  Base=${s.base_price:,.2f}  Bull=${s.bull_price:,.2f}  P(gain)={s.probability_of_gain:.1%}"
    )
print(
    f"  Path lengths: bear={len(proj.bear_path)}, base={len(proj.base_path)}, bull={len(proj.bull_path)}"
)
...
for p in result.projections:
    print(
        f"    {p.horizon_days:2d}d: ${p.bear_price:,.2f} — ${p.base_price:,.2f} — ${p.bull_price:,.2f}  ({p.expected_return:+.2%})"
    )
...
print(
    f"  Calibration: {result.model_selection.validation_summary.calibration_score:.4f}"
)
print("\n  Explanation:")
