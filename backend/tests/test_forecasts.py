...
def test_model_pipeline(model_class, model_name: str, p_keys: dict, prices, returns, features):
    ...
    for horizon in [1, 3, 7]:
        ...
        assert "expected_volatility" in pred, f"Missing expected_volatility for {model_name}"
        assert isinstance(pred["expected_return"], float)
        assert isinstance(pred["expected_volatility"], float)
        assert pred["expected_volatility"] > 0.0, f"Expected volatility is non-positive: {pred['expected_volatility']}"
