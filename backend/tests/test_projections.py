...
def test_path_generation():
    ...
    simulator = MonteCarloSimulator(spot=spot, drift=drift, volatility=vol, num_paths=num_paths, steps=steps)
    paths = simulator.generate_paths()
    
    # 1. Assert shape
    assert paths.shape == (num_paths, steps + 1), f"Unexpected shape of paths matrix: {paths.shape}"
...
def test_quantile_scenarios():
    ...
    spot = 100.0
    simulator = MonteCarloSimulator(spot=spot, drift=0.05, volatility=0.20, num_paths=10000, steps=7)
    paths = simulator.generate_paths()
...
def test_kde_density_mesh():
    ...
    simulator = MonteCarloSimulator(spot=100.0, drift=0.05, volatility=0.20, num_paths=10000, steps=7)
    paths = simulator.generate_paths()
    
    critical_steps = [1, 3, 7]
    grids = simulator.calculate_density_grid(paths, step_indices=critical_steps, grid_points=20)
