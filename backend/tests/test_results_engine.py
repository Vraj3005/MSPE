...
def test_result_engine_output_shape():
    """Verify that the result engine v2 output matches the expected schema."""
    print("  - Running Result Engine v2 output shape verification...")
    btc_result = ResultEngineService._run_demo_pipeline("BTCUSDT")

    assert btc_result.asset == "BTCUSDT"
    assert btc_result.asset_name == "Bitcoin / Tether USDT"
    assert btc_result.asset_class == "CRYPTO"
    assert btc_result.latest_price > 0
    assert btc_result.is_demo is True

    # Projections
    assert len(btc_result.projections) == 4
    for p in btc_result.projections:
        assert p.bear_price > 0
        assert p.base_price > 0
        assert p.bull_price > 0
        assert p.bear_price <= p.base_price <= p.bull_price

    # Risk
    assert btc_result.risk.risk_score > 0
    assert btc_result.risk.risk_level in ["Low", "Medium", "High", "Extreme"]
    assert btc_result.risk.var_95 > 0
    assert btc_result.risk.cvar_95 > 0

    # Model selection
    assert btc_result.model_selection.selected_model != ""
    assert btc_result.model_selection.models_compared > 0
    assert btc_result.model_selection.model_reason != ""

    # Explanation
    assert btc_result.explanation.what_mspe_expects != ""
    assert btc_result.explanation.why_this_result != ""
    assert btc_result.explanation.what_risk_to_watch != ""
    assert btc_result.explanation.how_reliable != ""

    # Explainability layer
    assert btc_result.explainability.summary != ""
    assert btc_result.explainability.model_reason != ""
    assert btc_result.explainability.risk_reason != ""
    assert btc_result.explainability.baseline_comparison != ""
    assert btc_result.explainability.reliability_label in ["High", "Medium", "Low"]
...
def test_api_response_schema_projections():
    ...
    assert len(density["prices"]) == 30
    assert len(density["densities"]) == 30
