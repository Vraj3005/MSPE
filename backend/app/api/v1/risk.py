...
@router.get("/portfolio/latest", response_model=risk_schemas.PortfolioRiskMetricsResponse)
async def get_latest_portfolio_risk(db: AsyncSession = Depends(get_db)):
    ...
    query = select(PortfolioRiskMetrics).order_by(desc(PortfolioRiskMetrics.timestamp)).limit(1)
...
async def list_assets_risk_metrics(db: AsyncSession = Depends(get_db)):
    ...
    latest_port_query = select(PortfolioRiskMetrics.timestamp).order_by(desc(PortfolioRiskMetrics.timestamp)).limit(1)
    ...
    query = select(AssetRiskMetrics, Asset.ticker).join(Asset).where(
        AssetRiskMetrics.timestamp == latest_time
    )
    result = await db.execute(query)
    rows = result.all()
...
async def get_assets_correlation_matrix(db: AsyncSession = Depends(get_db)):
    ...
    return risk_schemas.CorrelationMatrixGrid(
        assets=tickers,
        matrix=grid
    )
...
async def get_portfolio_stress_testing(db: AsyncSession = Depends(get_db)):
    ...
    for name, s_data in results.items():
        ...
        ticker_shocks = {k.replace("_shock", ""): v for k, v in s_data.items() if k.endswith("_shock")}
        
        scenarios.append(risk_schemas.StressScenarioResult(
            scenario_name=name,
            spx_shock=s_data.get("SPX_shock", 0.0) if "SPX_shock" in s_data else ticker_shocks.get("SPX", 0.0),
