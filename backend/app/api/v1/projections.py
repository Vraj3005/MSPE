...
async def get_latest_probabilistic_surface(ticker: str, db: AsyncSession = Depends(get_db)):
    ...
    async def run_projection_task():
        ...
        async with async_session_maker() as background_db:
            try:
                ...
                logger.error(f"Background Monte Carlo simulation failed for {ticker}: {e}")
