...
async def test_etl_ingestion_suite():
    ...
    async with async_session_maker() as db:
        ...
        assert len(assets) == 4, f"Seeding failed to register 4 standard assets, got {len(assets)}"
        ...
        for asset in assets:
            ...
            try:
                ...
                await IngestionService.backfill_asset(db, asset.ticker, days=15, resolution="1d")
                
                # Verify rows stored
                bars_query = select(func.count(MarketBar.timestamp)).where(
                    MarketBar.asset_id == asset.id,
                    MarketBar.resolution == "1d"
                )
                bars_result = await db.execute(bars_query)
                bars_count = bars_result.scalar()
                
                logger.info(f"Verification Check: Asset {asset.ticker} has {bars_count} historical daily bars saved.")
                assert bars_count > 0, f"Backfill failed to insert any bars for {asset.ticker}"
                
                # Fetch sample bars to verify details
                sample_query = select(MarketBar).where(
                    MarketBar.asset_id == asset.id
                ).order_by(MarketBar.timestamp.desc()).limit(1)
                
                sample_result = await db.execute(sample_query)
                latest_bar = sample_result.scalar_one_or_none()
                
                if latest_bar:
                    logger.info(f"Latest Bar Data stored:")
                    logger.info(f"  - Timestamp: {latest_bar.timestamp}")
                    logger.info(f"  - OHLCV: {latest_bar.open} | {latest_bar.high} | {latest_bar.low} | {latest_bar.close} | Vol: {latest_bar.volume}")
                    logger.info(f"  - Timezone Offset: {latest_bar.timestamp.tzinfo}")
                    
                    # Verify timezone is UTC
                    assert latest_bar.timestamp.tzinfo == timezone.utc, f"Timezone is not offset-aware UTC for {asset.ticker}"
                    
            except Exception as ex:
                logger.exception(f"Ingestion pipeline failed for asset {asset.ticker}: {ex}")
