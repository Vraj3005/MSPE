...
class ProjectionRun(Base):
    ...
    model_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("model_metadata.id", ondelete="CASCADE"), nullable=False
    )
    ...
    projected_surfaces = relationship(
        "ProjectedSurface", back_populates="run", cascade="all, delete-orphan"
    )
...
class ProjectedSurface(Base):
    ...
    run_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("projection_runs.id", ondelete="CASCADE"), nullable=False
    )
    ...
    p10_price: Mapped[float] = mapped_column(
        Numeric(18, 6), nullable=False
    )  # Bear price path value
    p50_price: Mapped[float] = mapped_column(
        Numeric(18, 6), nullable=False
    )  # Base price path value
    p90_price: Mapped[float] = mapped_column(
        Numeric(18, 6), nullable=False
    )  # Bull price path value
