...
# Add repository root to python path dynamically
...
from backend.app.api.v1 import validation as validation_api
...
async def run_initial_sync():
    ...
    async with async_session_maker() as db:
        try:
            ...
            logger.error(
                f"Initial startup synchronization warning: {e}. Checking again on scheduled intervals."
            )
...
async def lifespan(app: FastAPI):
    # --- STARTUP CYCLE ---
    logger.info(
        f"Starting Market Surface Projection Engine (MSPE) - Env: {settings.ENV}"
    )
    ...
    scheduler.start()
    logger.info(
        f"Incremental Ingestion Scheduler successfully started with intervals of {settings.INCREMENTAL_SYNC_INTERVAL_SECONDS} seconds."
    )
