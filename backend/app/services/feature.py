...
class FeatureService:
    @classmethod
    def calculate_features_dataframe(
        cls, df: pd.DataFrame, resolution: str
    ) -> pd.DataFrame:
        ...
        dx = (
            ...
            * np.abs(plus_di - minus_di)
            / np.where((plus_di + minus_di) == 0.0, 1e-9, plus_di + minus_di)
        )
        ...
        df["historical_volatility_30"] = (
            df["log_returns"].rolling(window=30).std() * ann_factor
        )
        ...
        df["parkinson_volatility_30"] = (
            np.sqrt(rolling_sum / (parkinson_constant * 30)) * ann_factor
        )
    ...
    async def compute_and_store_features(
        ...
        logger.info(
            f"Computing engineered features for asset: {asset_id} ({resolution})..."
        )
        ...
        if len(bars) < 30:
            logger.warning(
                f"Insufficient pricing history to calculate features for {asset_id} (found {len(bars)}/30 bars)"
            )
            return
        ...
        logger.info(
            f"Saving {len(df_features)} computed feature records to the database..."
        )
        values = []
        ...
        if values:
            ...
            stmt = stmt.on_conflict_do_update(
                constraint="market_features_pkey", set_=update_dict
            )
            await db.execute(stmt)

        await db.commit()
        logger.info(
            f"Features computation and upsert completed successfully for {asset_id} ({resolution})."
        )
