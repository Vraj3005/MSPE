...
class IngestionService:
    ...
    async def fetch_historical_data(
        ...
        if source == "BINANCE":
            return await cls._fetch_binance_klines(
                external_symbol, start_time, end_time, resolution
            )
        elif source == "YFINANCE":
            return await cls._fetch_yfinance_history(
                external_symbol, start_time, end_time, resolution
            )
        else:
    ...
    def validate_and_clean_bars(
        cls, raw_bars: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        for bar in raw_bars:
            ...
            if (
                ...
                logger.warning(
                    f"Dropped bar at {ts} due to invalid non-positive prices or volume: {bar}"
                )
                continue
            ...
            if not (high_p >= low_p):
                logger.warning(
                    f"Corrected anomaly at {ts}: High ({high_p}) was less than Low ({low_p}). Swapping bounds."
                )
    ...
    def handle_missing_values(
        cls, bars: List[Dict[str, Any]], asset_class: str, resolution: str
    ) -> List[Dict[str, Any]]:
    ...
    async def backfill_asset(
        cls, db: AsyncSession, ticker: str, days: int = 365, resolution: str = "1d"
    ) -> None:
        ...
        if not asset:
            raise ValueError(
                f"Asset does not exist in DB: {ticker}. Please ensure catalog is seeded."
            )
        ...
        logger.info(
            f"Executing historical backfill for {ticker} ({days} days at {resolution})..."
        )
        ...
        processed_bars = cls.handle_missing_values(
            valid_bars, meta["asset_class"], resolution
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to calculate features for {ticker} post-backfill: {e}"
            )
    ...
    async def sync_incremental(cls, db: AsyncSession, resolution: str = "1d") -> None:
        ...
        for meta in DEFAULT_ASSETS:
            ...
            query = (
                select(MarketBar)
                .where(
                    MarketBar.asset_id == asset.id, MarketBar.resolution == resolution
                )
            ...
            if not latest_bar:
                logger.info(
                    f"No existing records found for {meta['ticker']}. Redirecting to backfill..."
                )
                await cls.backfill_asset(
                    db, meta["ticker"], days=365, resolution=resolution
                )
                continue
            ...
            if end_time - start_time < min_delta:
                logger.info(
                    f"Asset {meta['ticker']} is already synchronized up to {start_time}"
                )
                continue
            ...
            try:
                ...
                logger.error(
                    f"Failed to calculate features for {meta['ticker']} post-sync: {e}"
                )
