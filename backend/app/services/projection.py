...
class ProjectionService:
    ...
    async def run_surface_projection(
        ...
        logger.info(f"Initiating Monte Carlo probabilistic surface projection for: {ticker}...")
        ...
        if not latest_bar:
            raise ValueError(f"No pricing bars found for asset {ticker}. Ingestion is required first.")
        ...
        ).order_by(desc(MarketForecast.timestamp)).limit(3) # Get 1, 3, 7 day forecasts
        ...
        if fc_rows:
            ...
            logger.info(f"Loaded parameters from forecast model {model_meta.model_type}: drift={drift:.4f}, volatility={volatility:.4f}")
        else:
            # Fallback model selection: create mock model metadata since no trained model exists
            logger.warning(f"No trained forecasting models found for {ticker}. Running with statistical default estimators.")
            
            # Seed a dummy model metadata record to maintain database reference constraints
            meta_stmt = insert(ModelMetadata).values(
        ...
        run_stmt = insert(ProjectionRun).values(
            asset_id=asset.id,
            model_id=model_id,
            timestamp=datetime.now(timezone.utc),
            parameters={"num_paths": num_paths, "steps": steps, "drift": drift, "volatility": volatility},
            status="RUNNING"
        ...
        try:
            ...
            density_grids = simulator.calculate_density_grid(paths, step_indices=critical_steps, grid_points=20)
            ...
            for grid_data in density_grids:
                ...
                for p_idx in range(len(prices_arr)):
                    ...
                    surf_stmt = insert(ProjectedSurface).values(
                        run_id=proj_run.id,
            ...
            await db.commit()
            logger.info(f"Surface projection run successfully committed to DB! (Duration: {duration:.2f}s)")
    ...
    @classmethod
    async def get_latest_projection_response(cls, db: AsyncSession, ticker: str) -> Optional[Dict[str, Any]]:
        ...
        ).order_by(ProjectedSurface.projection_time.asc(), ProjectedSurface.price.asc())
        
        surf_result = await db.execute(surf_query)
        surfaces = surf_result.scalars().all()

        # Load parent forecast model type
        model_query = select(ModelMetadata).where(ModelMetadata.id == latest_run.model_id)
