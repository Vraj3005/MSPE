...
class RiskService:
    ...
    async def get_asset_returns(
        cls, db: AsyncSession, asset_id: Any, resolution: str = "1d", lookback: int = 252
        ...
        ).order_by(MarketBar.timestamp.desc()).limit(lookback + 1) # get 1 extra bar for returns computation
    ...
    async def evaluate_risk_analytics(cls, db: AsyncSession) -> Dict[str, Any]:
        ...
        if not assets:
            logger.warning("No active assets registered in database. Aborting risk analytics evaluation.")
            return {}
        ...
        if benchmark_asset:
            benchmark_prices, benchmark_returns = await cls.get_asset_returns(db, benchmark_asset.id)
            logger.info(f"Loaded benchmark index (SPX) returns: {len(benchmark_returns)} bars.")
        else:
            logger.warning("Benchmark asset (SPX) not found in catalog. Sizing systematic metrics relative to equal index.")
        ...
        for asset in assets:
            logger.info(f"Computing risk and performance metrics for asset: {asset.ticker}...")
            ...
            if len(returns) == 0:
                logger.warning(f"No price history found for asset {asset.ticker}. Skipping risk calculation.")
                continue
            ...
            if len(benchmark_returns) > 0 and len(returns) > 0:
                ...
                beta, alpha = risk_calc.calculate_beta_alpha(returns[-min_len:], benchmark_returns[-min_len:])
            else:
                beta, alpha = 1.0, 0.0
            ...
            asset_metrics_records.append({
                ...
                "details": {
                    ...
                    "annualized_volatility": round(np.std(returns) * np.sqrt(252.0), 6),
                    "parametric_var_95": round(risk_calc.calculate_var_parametric(returns, 0.95), 6),
                    "sample_size_days": len(returns)
                }
            })

        # 4. Formulate portfolio weights from active trading signals
        active_signals_query = select(TradingSignal).where(TradingSignal.is_active == True)
        ...
        if active_size_usd > 0.0:
            for sig in active_signals:
                ...
                asset_res = await db.execute(select(Asset).where(Asset.id == sig.asset_id))
            ...
            logger.info(f"Constructed dynamic active signals portfolio weights: {weights}")
        else:
            # Fallback to equal weights across all active assets
            eq_w = 1.0 / len(assets)
            for asset in assets:
                weights[asset.ticker] = eq_w
            logger.info(f"No active positions found. Defaulting to equal portfolio weights: {weights}")

        # 5. Formulate Historical Portfolio Returns
        # Find minimum length of available active assets returns
        tickers_with_data = [t for t, ret in asset_returns_dict.items() if len(ret) > 0 and t in weights]
        ...
        if len(portfolio_returns) > 0:
            ...
            p_beta = float(sum(weights.get(m["ticker"], 0.0) * m["beta"] for m in asset_metrics_records))
            p_alpha = float(sum(weights.get(m["ticker"], 0.0) * m["alpha"] for m in asset_metrics_records))
        else:
        ...
        for am in asset_metrics_records:
            stmt = insert(AssetRiskMetrics).values(
        ...
        portfolio_stmt = insert(PortfolioRiskMetrics).values(
            timestamp=now,
            var_95=p_var_95,
            var_99=p_var_99,
            ...
            details={
                ...
                "parametric_var_95": round(risk_calc.calculate_var_parametric(portfolio_returns, 0.95) if len(portfolio_returns) > 0 else 0.0, 6)
            }
        ...
        await db.commit()
        logger.info("MSPE Risk Analytics evaluation successfully calculated and committed.")
        
        return {
            "portfolio": saved_portfolio,
            "assets_risk": saved_assets_res
        }
