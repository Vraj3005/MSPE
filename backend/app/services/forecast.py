...
from backend.quant.ml.stats_models import ARIMAForecaster, SARIMAForecaster, GARCHForecaster
...
class ForecastingService:
    ...
    async def train_and_persist_model(
        ...
        """
        logger.info(f"Initiating training pipeline for {ticker} using {model_type} ({resolution})...")
        
        asset = await IngestionService.get_asset_by_ticker(db, ticker)
        if not asset:
            raise ValueError(f"Asset with ticker {ticker} not found. Catalog seeding required.")

        if hyperparameters is None:
            hyperparameters = {}

        # 1. Fetch raw prices and engineered features
        prices, returns, features_mat, timestamps = await cls._load_historical_datasets(db, asset.id, resolution)
        if len(prices) < 40:
            raise ValueError(f"Insufficient history ({len(prices)}/40 bars) to train {model_type}")
        ...
        logger.info(f"Validation metrics calculated: RMSE={rmse:.6f} | Confidence Score={confidence_score:.4f}")
        ...
        logger.info(f"Generating forward price projections...")
        forecasts = []
        ...
        for horizon in [1, 3, 7]:
            ...
            fc_stmt = insert(MarketForecast).values(
    ...
    async def _load_historical_datasets(
        ...
        feat_query = select(MarketFeature).where(
            and_(MarketFeature.asset_id == asset_id, MarketFeature.resolution == resolution)
        ...
        for bar in bars:
            ...
            vol = f_row.historical_volatility_30 if f_row.historical_volatility_30 is not None else 0.20
            ...
            aligned_returns.append(float(f_row.returns_1d) if f_row.returns_1d is not None else 0.0)
    ...
    async def _execute_walk_forward_validation(
        ...
        logger.info(f"Running chronological walk-forward validation (steps={validation_steps})...")
        ...
        for i in range(validation_steps):
            ...
            try:
                ...
                model = cls._instantiate_model(model_type, hyperparameters)
                model.fit(prices=train_prices, returns=train_returns, features=train_features)
                ...
                logger.warning(f"Validation step {i} failed: {e}. Skipping step metric.")
