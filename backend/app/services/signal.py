...
MAX_PORTFOLIO_RISK = 0.05     # 5% maximum aggregate portfolio risk ceiling ($5,000 USD risk)
...
class SignalService:
    ...
    async def evaluate_signals(cls, db: AsyncSession) -> List[TradingSignal]:
        ...
        active_assets = await db.execute(select(Asset).where(
        ...
        for asset in assets:
            ...
            spot_query = select(MarketBar).where(
            ...
            forecast_query = select(MarketForecast, ModelMetadata).join(ModelMetadata).where(
                and_(
                    MarketForecast.asset_id == asset.id,
                    ModelMetadata.is_active == True
                )
            ).order_by(desc(MarketForecast.timestamp)).limit(3)
            ...
            one_day_fc = next((f for f in forecasts if f.horizon_days == 1), forecasts[0])
            ...
            run_query = select(ProjectionRun).where(
                and_(ProjectionRun.asset_id == asset.id, ProjectionRun.status == "COMPLETED")
            ).order_by(desc(ProjectionRun.timestamp)).limit(1)
            ...
            surf_query = select(ProjectedSurface).where(
                ProjectedSurface.run_id == latest_run.id
            ).order_by(desc(ProjectedSurface.projection_time)).limit(1) # terminal step
            ...
            if sig_type == "NO_TRADE":
                ...
                raw_signals.append({
                    ...
                    "rank_score": 0.0,
                    "details": {"reason": "Return threshold or barrier convergence constraints not satisfied"}
                })
                continue
            ...
            if rrr < 1.5:
                raw_signals.append({
                    ...
                    "confidence_score": confidence_score,
                    "rank_score": 0.0,
                    "details": {"reason": f"Risk-Reward Ratio {rrr:.2f} was less than standard minimum 1.5 filter"}
                })
                continue
            ...
            position_size_usd = min(PORTFOLIO_EQUITY * 0.5, position_size_usd) # Max size 50% equity for single trade
        ...
        active_candidates = [s for s in raw_signals if s["signal_type"] in ["LONG", "SHORT"]]
        # Sort candidates in descending order based on composite Rank Score
        active_candidates = sorted(active_candidates, key=lambda x: x["rank_score"], reverse=True)
        ...
        for cand in active_candidates:
            ...
            percent_loss = abs(cand["entry_price"] - cand["stop_loss"]) / cand["entry_price"]
            ...
            if current_risk_usd + trade_risk <= max_allowed_risk_usd:
                ...
                logger.info(f"Signal executed for {cand['ticker']} with size ${cand['position_size']:.2f}")
            else:
                # Cap is breached, reject signal to protect portfolio equity
                details["risk_status"] = "REJECTED_PORTFOLIO_RISK_CEILING_BREACHED"
                logger.warning(f"Rejected signal for {cand['ticker']} due to aggregate portfolio risk limit reached.")
        ...
        await db.commit()
        logger.info(f"Trading signals evaluation successfully committed. Open active signals count: {len([s for s in saved_signals if s.is_active])}")
