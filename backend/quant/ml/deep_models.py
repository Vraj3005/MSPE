...
class LSTMForecaster(BaseForecaster):
    def __init__(self, epochs: int = 20, lr: float = 0.01, seq_len: int = 10, hidden_dim: int = 32):
        self.epochs = epochs
        self.lr = lr
    ...
    def fit(self, prices: np.ndarray, returns: np.ndarray, features: Optional[np.ndarray] = None) -> "LSTMForecaster":
        ...
        self.feature_stds = np.where(self.feature_stds == 0.0, 1e-9, self.feature_stds) # prevent div by zero
        ...
        for i in range(n - self.seq_len - 3):
            ...
            X_seq.append(seq)
            y_target.append([target_ret[i + self.seq_len - 1], target_vol[i + self.seq_len - 1]])

        if len(X_seq) == 0:
            logger.warning("Data length too short to frame sequence paths. Fits cancelled.")
            return self
    ...
    def predict(self, horizon: int) -> Dict[str, float]:
        ...
        try:
            self.network.eval()
            with torch.no_grad():
                X_inf = torch.tensor(self.latest_sequence, dtype=torch.float32).unsqueeze(0).to(self.device) # shape (1, SeqLen, InputDim)
