...
class ARIMAForecaster(BaseForecaster):
    ...
    def fit(
        ...
        logger.info(
            f"Fitting ARIMA({self.p},{self.d},{self.q}) on {len(prices)} bars..."
        )
        try:
            ...
            logger.warning(
                f"ARIMA fit failed to converge: {e}. Falling back to simple historical estimators."
            )
            self.model_res = None
        return self
    ...
    def predict(self, horizon: int) -> Dict[str, float]:
        if self.model_res is None or len(self.history_prices) == 0:
            ...
            return {
                ...
                "expected_volatility": float(
                    np.std(self.history_returns[-30:]) * np.sqrt(252)
                ),
            }
        ...
        try:
            ...
            expected_volatility = (
                float((np.std(residuals[-30:]) / current_price) * np.sqrt(252))
                if current_price > 0
                else 0.20
            )
...
class SARIMAForecaster(BaseForecaster):
    ...
    def fit(
        ...
        logger.info(
            f"Fitting SARIMA({self.p},{self.d},{self.q})x({self.P},{self.D},{self.Q}){self.s}..."
        )
        try:
            model = SARIMAX(
                prices,
...
class GARCHForecaster(BaseForecaster):
    ...
    def fit(
        ...
        logger.info(
            f"Fitting GARCH({self.p},{self.q}) volatility model on {len(returns)} bars..."
        )
        try:
            model = arch_model(
                scaled_returns,
            ...
            logger.warning(
                f"GARCH calibration failed: {e}. Falling back to historical standard deviation."
            )
            self.model_res = None
        return self
    ...
    def predict(self, horizon: int) -> Dict[str, float]:
        ...
        try:
            ...
            mu = (
                float(
                    self.model_res.params.get(
                        "mu", np.mean(self.history_returns) * 100.0
                    )
                )
                / 100.0
            )
