...
class XGBoostForecaster(BaseForecaster):
    def __init__(
        self, n_estimators: int = 100, max_depth: int = 3, learning_rate: float = 0.05
    ):
    ...
    def fit(
        ...
        if features is None or len(features) < 15:
            logger.warning(
                "Features array not provided or too short. Generating fallback lag features."
            )
        ...
        self.latest_features = features[
            -1
        ]  # Store latest features for forward inference

        logger.info(f"Training XGBoost double-heads on {len(X)} samples...")
        try:
            self.model_ret = XGBRegressor(
            ...
            logger.error(
                f"XGBoost fit failed: {e}. Falling back to default estimators."
            )
            self.model_ret = None
            self.model_vol = None
        return self
    ...
    def tune_hyperparameters(
        self, X: np.ndarray, y_ret: np.ndarray, y_vol: np.ndarray
    ) -> None:
        ...
        try:
            ...
            logger.info(
                f"Optimal parameters found: n_estimators={self.n_estimators}, max_depth={self.max_depth}"
            )
        except Exception as e:
            logger.warning(
                f"Hyperparameter tuning failed: {e}. Reverting to standard defaults."
            )
    ...
    def predict(self, horizon: int) -> Dict[str, float]:
        ...
        if (
            self.model_ret is None
            or self.model_vol is None
            or self.latest_features is None
        ):
...
class RandomForestForecaster(BaseForecaster):
    ...
    def fit(
        ...
        try:
            self.model_ret = RandomForestRegressor(
                n_estimators=self.n_estimators,
                max_depth=self.max_depth,
                random_state=42,
            )
            ...
            self.model_vol = RandomForestRegressor(
                n_estimators=self.n_estimators,
                max_depth=self.max_depth,
                random_state=42,
            )
