...
def calculate_sharpe_ratio(
    returns: np.ndarray, risk_free_rate_annual: float = 0.02
) -> float:
...
def calculate_calmar_ratio(
    prices: np.ndarray, annualized_return: float, risk_free_rate_annual: float = 0.02
) -> float:
...
def calculate_beta_alpha(
    ...
    if (
        ...
        or len(asset_returns) != len(benchmark_returns)
    ):
        return 1.0, 0.0
    ...
    alpha = float(
        mean_asset
        - (risk_free_rate_annual + beta * (mean_benchmark - risk_free_rate_annual))
    )
...
def run_portfolio_stress_test(
    ...
    """
    scenarios = {
        "2008_GFC": {"BTCUSDT": -0.65, "ETHUSDT": -0.70, "SPX": -0.40, "XAU": 0.10},
        "COVID_CRASH_2020": {
            "BTCUSDT": -0.50,
            "ETHUSDT": -0.55,
            "SPX": -0.30,
