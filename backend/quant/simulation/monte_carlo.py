...
class MonteCarloSimulator:
    ...
    def generate_paths(self) -> np.ndarray:
        ...
        """
        logger.info(
            f"Generating {self.num_paths} Monte Carlo paths over {self.steps} steps..."
        )
    ...
    def extract_scenarios(self, paths: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        return {"bear_scenario": p10, "base_scenario": p50, "bull_scenario": p90}
    ...
    def calculate_density_grid(
        ...
        """
        logger.info(
            f"Computing continuous Kernel Density Estimations (KDE) at steps {step_indices}..."
        )
        ...
        for step in step_indices:
            ...
            norm_densities = (
                densities / sum_densities if sum_densities > 0.0 else densities
            )
